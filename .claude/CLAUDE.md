...
The README and `docs/CASE_STUDY.md` are read by recruiters and hiring managers, not by
maintainers. Write them as evidence that the author can design and run a segmented
network. Keep repository process out of them: no monorepo-extraction history, no
"which claim was corrected", no open-decisions list, no tooling commentary. Those
belong in `PROJECT_NOTES.md`, which is deliberately untracked (see `.git/info/exclude`)
and stays local.
